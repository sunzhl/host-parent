...
create table if not exists `structures`(
  ...
  `struct_number` varchar(10) not null unique comment '建筑物编号',
  ...
  `pro_number` varchar(10) not null comment '所属建筑物编号',
...
create table if not exists `work_tools`(
  ...
  `tool_code` varchar(10) not null comment '运转工具编号',
...
create table if not exists `worker_info`(
   ...
   `struct_number` varchar(10) not null  comment '责任建筑编号',
...
create table if not exists `sys_users`(
   ...
   `state` tinyint(1) not null default 1 comment '状态 0-不可用, 1-可用，2-被锁'
);
...
create table if not exists `task_type`(
  ...
  `type_number` varchar(10) NOT NULL UNIQUE COMMENT '类型编号',
  `type_name` varchar(20) not null comment '类型名称',
  `parent_number` varchar(10) not null default '' comment '父类型',
